/* =========================================================================
   ...
   Shared behaviour for every page (no framework, no build step):
     ...
     2. header + footer injection  (#site-header / #site-footer)
   ...
   Requires /assets/js/data.js to be loaded first.
   ========================================================================= */
(function () {
  'use strict';
  ...
  function renderHeader() {
    var mount = qs('#site-header');
    if (!mount) return;
    var cats = MZ.categories || [];
    var nav = MZ.nav || [];

    var tabs = nav.map(function (item) {
      var active = isActive(item.href);
      var inner = '<span class="icon-wrapper">' + icon(item.icon) + '</span><span>' + esc(item.label) + '</span>' +
        (item.badge ? '<span class="new-badge">' + esc(item.badge) + '</span>' : '') +
        '<span class="active-tab-underline" aria-hidden="true"></span>'; // Paragon's JS-measured underline glow, shown only on .active
      var current = active ? ' aria-current="page"' : '';
      if (item.dropdown === 'categories') {
        return '<div class="nav-dropdown-wrap">' +
          '<a class="nav-tab has-dropdown' + (active ? ' active' : '') + '" href="' + item.href + '" aria-haspopup="true" aria-expanded="false" aria-controls="packs-dropdown"' + current + '>' + inner + '</a>' +
          '<div class="dropdown-panel" id="packs-dropdown" aria-label="Pack categories">' +
          cats.map(function (c) {
            return '<a class="nav-tab" href="' + c.href + '" style="--color:' + c.color + '">' +
              '<span class="dot' + (c.gradient ? ' dot--gradient' : '') + '" aria-hidden="true"></span><span>' + esc(c.title) + '</span></a>';
          }).join('') +
          '</div></div>';
      }
      return '<a class="nav-tab' + (active ? ' active' : '') + '" href="' + item.href + '"' + current + '>' + inner + '</a>';
    }).join('');

    var mobile = nav.map(function (item) {
      var active = isActive(item.href);
      var html = '<a class="mobile-menu-item' + (active ? ' active' : '') + '" href="' + item.href + '"' + (active ? ' aria-current="page"' : '') + '>' +
        icon(item.icon) + '<span>' + esc(item.label) + '</span>' + (item.badge ? '<span class="new-badge">' + esc(item.badge) + '</span>' : '') + '</a>';
      if (item.dropdown === 'categories') {
        html += '<div class="mobile-menu-sub">' + cats.map(function (c) {
          return '<a class="title-tag nav-tag" href="' + c.href + '" style="--color:' + c.color + '">' + esc(c.title) + '</a>';
        }).join('') + '</div>';
      }
      return html;
    }).join('');
    ...
    mount.innerHTML =
      '<div class="nav-container">' +
        '<nav class="nav" aria-label="Primary">' +
          '<div class="nav-left">' +
            '<a href="/" class="logo-container nav-pill" aria-label="MIZTWEAKZ home">' +
              '<img src="/assets/img/brand/logo-96.png" width="30" height="30" alt="">' +
              '<span class="logo-text">MIZTWEAKZ</span>' +
            '</a>' +
            '<button type="button" class="hamburger-container nav-pill" aria-label="Open menu" aria-expanded="false" aria-controls="mobile-menu">' + icon('bars') + '</button>' +
          '</div>' +
          '<div class="tab-container nav-pill compact">' + tabs + '</div>' +
          '<div class="nav-user-controls">' +
            '<span class="nav-spacer" aria-hidden="true"></span>' +
          '</div>' +
        '</nav>' +
        '<nav class="mobile-menu glass-card" id="mobile-menu" aria-label="Mobile">' + mobile + '</nav>' +
      '</div>';
  ...
  function initNav(root) {
    var wrap = qs('.nav-dropdown-wrap', root);
    if (wrap) {
  ...
  /* ---------- footer ---------- */
  function socialBtn(name, href, label) {
    return '<a class="social-btn" href="' + href + '" target="_blank" rel="noopener noreferrer" aria-label="' + esc(label) + '">' + icon(name) + '</a>';
  }
  function renderFooter() {
    var mount = qs('#site-footer');
    if (!mount) return;
    var b = MZ.brand || {}, s = b.socials || {};
    var links = (MZ.nav || []).filter(function (i) { return i.href !== '/'; }).map(function (i) {
      return '<a class="nav-link" href="' + i.href + '">' + esc(i.label) + '</a>';
    }).join('');
    mount.innerHTML =
      '<footer class="footer">' +
        ...
        '<div class="footer-content">' +
          '<div class="footer-brand">' +
            '<a href="/" class="brand-link" aria-label="MIZTWEAKZ home"><img class="brand-logo" src="/assets/img/brand/logo-192.png" width="50" height="50" alt="MIZ TWEAKZ" loading="lazy"></a>' +
            '<p class="brand-tagline">' + esc(b.tagline) + '</p>' +
            '<div class="social-links">' +
              socialBtn('discord', s.discord, 'Discord') + socialBtn('youtube', s.youtube, 'YouTube') +
              socialBtn('tiktok', s.tiktok, 'TikTok') + socialBtn('instagram', s.instagram, 'Instagram') +
              
            '</div>' +
          '</div>' +
          '<div class="footer-nav-col"><h4 class="nav-title">Navigate</h4><nav class="footer-nav" aria-label="Footer">' + links + '</nav></div>' +
          '<div class="footer-nav-col"><h4 class="nav-title">Legal</h4><nav class="footer-nav" aria-label="Legal">' +
            '<a class="nav-link" href="/tos/">Terms of Service</a>' +
            '<a class="nav-link" href="/privacy/">Privacy Policy</a>' +
            '<button type="button" class="nav-link nav-link-action" data-modal="cookie-prefs">Cookie Preferences</button>' +
          '</nav></div>' +
          '<div class="footer-cta glass-card">' +
            '<h4 class="nav-title">Ready to optimize?</h4><p class="cta-text">Get your PC running at peak performance.</p>' +
            '<div class="nice-button-box"><a class="nice-button btn-md" href="' + ((MZ.download && MZ.download.href) || '/') + '">' + esc((MZ.download && MZ.download.label) || 'Download for Free').replace(/Free$/, '<span class="free-word">Free</span>') + '</a></div>' +
          '</div>' +
        '</div>' +
        '<div class="footer-bottom"><div class="bottom-content">' +
          '<div class="bottom-text">' +
            '<p class="copyright">© ' + (b.year || new Date().getFullYear()) + ' MIZTWEAKZ. All rights reserved.</p>' +
            '<p class="disclosure">Student redesign concept for a school project. Not affiliated with MIZTWEAKZ; all purchases link to the official store.</p>' +
          '</div>' +
